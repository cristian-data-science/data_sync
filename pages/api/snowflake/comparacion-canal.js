...
import { buildComparacionPorCanalQuery } from '../../../backend/src/queries/snowflake-queries.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const query = buildComparacionPorCanalQuery({ fromDate: dateRange.from, toDate: dateRange.to });
    ...
    res.status(statusCode).json({ success: false, message: error.message || 'Error ejecutando comparación por canal' });
  }
}
