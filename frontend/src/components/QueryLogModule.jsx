...
const ACTION_LABELS = {
  ...
  "rollback-from-log": "Rollback desde logs",
};

const ACTION_COLORS = {
  sql: "purple",
  rollback: "orange",
  "rollback-from-log": "teal",
};
...
const copyText = async (text, toast, label) => {
  ...
  try {
    ...
    toast({
      title: `${label} copiado`,
      description: "Disponible en tu portapapeles.",
      status: "success",
      duration: 2500,
      isClosable: true,
    });
...
const parseMetadata = (raw) => {
  ...
  if (typeof raw === "string") {
    try {
      ...
      console.warn("No se pudo parsear el extraMetadata del log", error);
    }
  }
  return {};
};
...
const buildLogDescription = (log) => {
  ...
  const entryLabel =
    metadata.preview?.salesLinePk || metadata.entryId || log.entryId || metadata.entryID || null;
  const lineNumber = toNumeric(log.lineNumber) ?? toNumeric(metadata.lineNumber);
  const columns = describeColumns(
    Array.isArray(metadata.preview?.columnas)
      ? metadata.preview.columnas
      : Array.isArray(metadata.columnas)
        ? metadata.columnas
        : null
  );
  ...
  if (actionType === "rollback") {
    return `Se ejecutó el rollback manual para ${orderLabel} y se restauraron los datos originales.`;
  }

  if (kind === "insert") {
    return `Se insertó un registro para ${orderLabel} en Snowflake para reponer la línea faltante.`;
  }

  return columns
    ? `Actualizó ${columns} de ${target}.`
    : `Ejecutó SQL manual directamente sobre ${target}.`;
};
...
export default function QueryLogModule() {
  ...
  const logsQuery = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const { data } = await api.get("/api/snowflake/query-logs", { params });
      return data;
    },
  ...
  const rollbackMutation = useMutation({
    mutationFn: async ({ logId }) => {
      const { data } = await api.post(`/api/snowflake/query-logs/${logId}/rollback`);
      return data;
    },
    onMutate: ({ logId }) => {
      setExecutingRollbackId(logId);
    },
    onSuccess: (data) => {
      toast({
        title: "Rollback ejecutado",
        description: data?.message || "Snowflake confirmó la reversión.",
        status: "success",
        duration: 4000,
        isClosable: true,
      });
    ...
    onError: (error) => {
      toast({
        title: "No se pudo ejecutar el rollback",
  ...
  const handleConfirmRollback = () => {
    if (!pendingRollback || rollbackMutation.isLoading || confirmGuardRef.current) {
      return;
    }
    confirmGuardRef.current = true;
    setDialogSubmitting(true);
    rollbackMutation.mutate(
      { logId: pendingRollback.logId },
      {
        onError: () => {
          confirmGuardRef.current = false;
          setDialogSubmitting(false);
  ...
  const handleClearFilters = () => {
    setFormFilters({ salesId: "", actionType: "" });
    setAppliedFilters({ salesId: "", actionType: "" });
    setPage(0);
  };
  ...
  if (logsQuery.isLoading && !logsQuery.data) {
    return (
      <Box bg="white" borderRadius="2xl" shadow="md" p={12} textAlign="center">
        <VStack spacing={4}>
          <Spinner size="xl" color="purple.500" thickness="4px" />
          <Text fontSize="lg" fontWeight="semibold" color="gray.700">
            Cargando historial de Snowflake...
          </Text>
          <Text fontSize="sm" color="gray.500">
            Esto puede demorar unos segundos mientras consultamos los logs.
          </Text>
        </VStack>
      </Box>
    );
    ...
    <VStack align="stretch" spacing={6}>
      <Box bg="white" borderRadius="2xl" shadow="md" p={6} borderWidth="1px" borderColor="gray.100">
        <Stack direction={{ base: "column", md: "row" }} spacing={4} align="start">
          <InputGroup maxW="320px">
            <InputLeftAddon fontWeight="semibold" bg="gray.50">
              SalesId
            </InputLeftAddon>
            <Input
              placeholder="PAT-000000"
              value={formFilters.salesId}
              onChange={(event) => setFormFilters((prev) => ({ ...prev, salesId: event.target.value }))}
            />
          </InputGroup>
          <InputGroup maxW="220px">
            ...
            <Select
              value={formFilters.actionType}
              onChange={(event) => setFormFilters((prev) => ({ ...prev, actionType: event.target.value }))}
            >
          ...
          <HStack spacing={3}>
            <Button colorScheme="purple" onClick={handleApplyFilters} isLoading={logsQuery.isFetching}>
              Buscar
            </Button>
            ...
            <Tooltip label="Refrescar">
              <IconButton
                ...
                isLoading={logsQuery.isRefetching}
              />
            </Tooltip>
          </HStack>
        </Stack>
        ...
        <HStack justify="space-between" align={{ base: "stretch", md: "center" }} flexWrap="wrap" gap={3}>
          <Box>
            <Text fontWeight="bold" color="gray.700">
              {total ? `Mostrando ${startRow}-${endRow} de ${total} registros` : "Sin registros"}
            </Text>
            {total > 0 && (
              <Text fontSize="xs" color="gray.500">
                Última actualización: {new Date(logsQuery.data?.timestamp || Date.now()).toLocaleString("es-AR")}
              </Text>
            )}
          </Box>
          <HStack spacing={2}>
            <Button
              ...
              isDisabled={page === 0 || logsQuery.isFetching}
            >
              Anterior
            </Button>
            <Button
              ...
              isDisabled={!hasNext || logsQuery.isFetching}
            >
              Siguiente
            </Button>
          </HStack>
      ...
      {logsQuery.isError && (
        <Alert status="error" borderRadius="lg">
          <AlertIcon />
          {logsQuery.error?.message || "No se pudieron cargar los logs"}
        </Alert>
      )}
      ...
      {logsQuery.isLoading ? (
        ...
        <Box bg="white" borderRadius="2xl" shadow="md" p={8} textAlign="center">
          ...
          <Text fontSize="sm" color="gray.500" mt={2}>
            Ejecuta un script o ajusta los filtros para ver el historial.
          </Text>
        </Box>
      ) : (
        <VStack align="stretch" spacing={4}>
          {logs.map((log) => {
            ...
            const hasRollback = Boolean(
              log.rollbackSql && log.rollbackSql.trim() && log.rollbackSql.trim() !== "-- N/A"
            );
            return (
              <Box key={log.logId} borderWidth="1px" borderRadius="2xl" p={6} bg="white" shadow="sm">
                <HStack justify="space-between" align="start" flexWrap="wrap" gap={3}>
                  <Box>
                    ...
                    <Text fontSize="xs" color="gray.500">
                      {new Date(log.executedAt).toLocaleString("es-AR")}
                    </Text>
                  </Box>
                ...
                <Grid templateColumns={{ base: "repeat(1, 1fr)", md: "repeat(4, 1fr)" }} gap={3} mt={4} fontSize="sm">
                  <GridItem>
                    <Text color="gray.500">SalesId</Text>
                    <Text fontWeight="semibold">{log.salesId || "—"}</Text>
                  </GridItem>
                  <GridItem>
                    <Text color="gray.500">EntryId</Text>
                  ...
                  <GridItem>
                    <Text color="gray.500">Tipo/KIND</Text>
                  ...
                  <Box flex={1}>
                    <HStack justify="space-between" mb={2}>
                      ...
                      <Button
                        ...
                        onClick={() => copyText(log.executedSql, toast, "SQL")}
                      >
                        Copiar
                      </Button>
                    </HStack>
                  ...
                  <Box flex={1}>
                    <HStack justify="space-between" mb={2}>
                      ...
                      <Button
                        ...
                        variant="outline"
                        leftIcon={<HiClipboardCopy />}
                        onClick={() => copyText(log.rollbackSql, toast, "Rollback")}
                        isDisabled={!hasRollback}
                      >
                        Copiar
                      </Button>
                    </HStack>
                ...
                <ButtonGroup size="sm" spacing={3} mt={6} flexWrap="wrap">
                  <Button
                    ...
                    onClick={() => setPendingRollback(log)}
                    isDisabled={!hasRollback || rollbackMutation.isLoading}
                    isLoading={executingRollbackId === log.logId && rollbackMutation.isLoading}
      ...
      <AlertDialog
        isOpen={Boolean(pendingRollback)}
        leastDestructiveRef={cancelRef}
        onClose={() => (!(rollbackMutation.isLoading || dialogSubmitting) ? setPendingRollback(null) : null)}
        isCentered
      >
        <AlertDialogOverlay>
          <AlertDialogContent position="relative" overflow="hidden">
            ...
            <AlertDialogBody opacity={rollbackMutation.isLoading || dialogSubmitting ? 0.4 : 1} pointerEvents={rollbackMutation.isLoading || dialogSubmitting ? "none" : "auto"}>
              <Text mb={3} color="gray.700">
                Vas a ejecutar el rollback almacenado del log #{pendingRollback?.logId}. Esto revertirá la operación aplicada originalmente sobre SalesId {pendingRollback?.salesId || "desconocido"}.
              </Text>
              <Text fontSize="sm" color="gray.500">
                El backend ejecutará el SQL de rollback directamente en Snowflake usando las mismas credenciales.
              </Text>
            </AlertDialogBody>
            <AlertDialogFooter opacity={rollbackMutation.isLoading || dialogSubmitting ? 0.4 : 1} pointerEvents={rollbackMutation.isLoading || dialogSubmitting ? "none" : "auto"}>
              <Button ref={cancelRef} onClick={() => setPendingRollback(null)} disabled={rollbackMutation.isLoading || dialogSubmitting}>
                Cancelar
              </Button>
              <Button
                colorScheme="orange"
                ...
                ml={3}
                isLoading={rollbackMutation.isLoading || dialogSubmitting}
              >
                Sí, ejecutar rollback
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialogOverlay>
