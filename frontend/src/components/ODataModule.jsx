...
import { HiSearch, HiPencilAlt, HiX, HiCheck, HiDatabase } from "react-icons/hi";
...
function useSearch(salesId, enabled) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/api/odata/search`, { params: { salesId } });
      return data;
    },
  ...
  const fields = useMemo(
    () =>
      Object.keys(record || {}).filter(
        (k) => !k.startsWith("@") && typeof record[k] !== "object"
      ),
    [record]
  );
...
function ODataModule() {
  ...
  const mutation = useMutation({
    mutationFn: async (payload) => {
      const { data } = await api.patch(`/api/odata/update`, payload);
      return data;
    },
    onSuccess: () => {
    ...
    onError: (err) => {
      toast({
        ...
        description: err?.response?.data?.text || err.message,
        status: "error",
      });
    },
  });
  ...
  const onSave = (local) => {
    ...
    mutation.mutate({
      salesId: selected?.SalesId || selected?.SALESID || salesId,
      changes,
    });
  };
  ...
  const columns = useMemo(() => {
    ...
    return Object.keys(rows[0]).filter(
      (k) => !k.startsWith("@") && typeof rows[0][k] !== "object"
    );
  }, [rows]);
        ...
        <VStack align="stretch" spacing={4}>
          ...
          <HStack spacing={4}>
            ...
            <Button
              ...
              isDisabled={!salesId.trim() || isFetching}
        ...
        {isFetching ? (
          ...
          <>
            ...
            <Box maxH="500px" overflowY="auto" overflowX="auto">
              <Table variant="simple" size="sm">
                ...
                <Tbody>
                  {rows.map((r, i) => (
                    <Tr key={i} _hover={{ bg: "gray.50" }} transition="background 0.2s">
                      {columns.map((c) => (
                        <Td key={c} fontSize="sm" color="gray.700">
                          {String(r[c] ?? "")}
                        </Td>
                      ))}
                    </Tr>
          ...
          <VStack py={20} spacing={4}>
            ...
            <VStack spacing={2}>
              ...
              <Text fontSize="sm" color="gray.500">
                {searchTrigger
                  ? "Intenta con otro Sales ID"
                  : "Ingresa un Sales ID y presiona Buscar"}
              </Text>
            </VStack>
          </VStack>
        )}
